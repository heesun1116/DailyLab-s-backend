...
export const getPostById = async (ctx, next) => {
  ...
  try {
    const post = await Post.findById(id); //when post does not exist
...
export const list = async ctx => {
  ...
  const {
    tag,
    username
  } = ctx.query; // tag, put in object if username value is valid, or not
...
export const update = async ctx => {
  const {
    id
  } = ctx.params; // Similar to schema used by write, no required().
